        ...
        try:
            ...
            for key in context:
                if isinstance(context[key], QuerySet):
                    ...
                    context[key] = to_dict(context[key])
...
def login( request):
    ...
    if(request.method == 'POST'):
        ...
        exito, mensaje = authenticate(request)

        if(not exito):
            return handle_error(request, status=403, message=mensaje)
        ...
        return handle_error(request, status=400, message="Request inválido")
