    ...
    def guardar_subform_personal_Form(self, request):
        if self.is_roles(request.user, ["admin", "medico"]):
            if request.method == 'POST':
                ...
                form = Form.objects.get(id_form=request.GET['id_form'])
                subform = SubForm_historia_personal.objects.filter(pk=form.subform_hist_per.pk).get()

                subform = SubForm_historia_personal_Form(request.POST, instance=subform)
    ...
    def guardar_subForm_antecedentes_g_o(self, request):
        ...
        if self.is_roles(request.user, ["admin", "medico"]):
            if request.method == 'POST':
                ...
                subform_Form = SubForm_antecedentes_g_o_Form(request.POST, instance=subform)
                if subform_Form.is_valid():

                    model_subform = subform_Form.save(commit=False)

                    if not model_subform.parto_cantidad == None:
                        print(model_subform.parto_cantidad, type(model_subform.parto_cantidad))
                        if model_subform.parto_cantidad == 0:
                            model_subform.parto_aplica = False
                        else:
                            model_subform.parto_aplica = True
